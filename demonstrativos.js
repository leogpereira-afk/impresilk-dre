...
const dreUI={base:'caixa',detalhar:false};
const linhasCaixa=[
 ...
 ['variacao','(=) Variação do caixa considerado','result','Variação de caixa']
...
function valorDRE(v,ratio=false){return v==null?'<span class="muted" title="Não apurado: faltam informações">—</span>':ratio?v.toLocaleString('pt-BR',{maximumFractionDigits:1})+'%':money(v);}
function dadosDRE(){
...
function abrirCompetencia(label){
 ...
 const revisar=()=>{
  const calc=DREModelo.calcular(draft.valores),pendentes=DREModelo.campos.filter(c=>draft.valores[c.id]==null);
