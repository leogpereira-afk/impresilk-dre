...
test('rotas da coleta conferem o papel no servidor',async()=>{
  const s=server(null,{usuario:'leitura'}),reader={authorization:'Bearer '+jwt('usuario')};
  assert.equal((await s.call({action:'solicitarColeta'},reader)).status,403);
  assert.equal((await s.call({action:'coletaIniciar',runId:'123'},reader)).status,403);
  assert.equal((await s.call({action:'coletaConcluir',runId:'123',ok:true},reader)).status,403);
  assert.equal((await s.call({action:'solicitarColeta'},collector)).status,403);
  assert.equal((await s.call({action:'coletaStatus'},{})).status,401);
  assert.equal((await s.call({action:'coletaStatus'},reader)).body.status.ativa,false);
  assert.equal(s.writes(),0);
});
