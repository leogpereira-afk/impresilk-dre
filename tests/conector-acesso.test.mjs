...
function server({role='leitura',dbFail=false,response={status:200,data:[]}}={}) {
  ...
  const env={DRE_TOKEN:'fixture-machine',EQUIPE_JWT_SECRET:'fixture-jwt',MUBI_PUBLIC_KEY:'fixture-public',MUBI_TOKEN:'fixture-access'};
  ...
  return {writes:()=>writes,call:async(body,machine=false)=>{const r=await handler(new Request('https://test/dre-financas',{method:'POST',headers:{'content-type':'application/json',...(machine?{'x-token':env.DRE_TOKEN}:{authorization:'Bearer '+jwt})},body:JSON.stringify(body)}));return {status:r.status,body:await r.json()};}};
}
