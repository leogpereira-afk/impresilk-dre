...
function historicoTabela(){const year=state.periodo.split('/')[1];const recs=state.records.filter(r=>r.label.endsWith('/'+year));if(!recs.length)return '<p>Sem histórico neste ano.</p>';const total=recs.reduce((a,x)=>{const r=F.resumo(x);a.e+=r.entradas;a.s+=r.saidas;return a;},{e:0,s:0});
