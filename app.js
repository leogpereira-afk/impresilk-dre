...
function render(){
  ...
  const pages={dre:renderDRE,glossario:renderGlossario,inicio:renderInicio,caixa:renderCaixa,resultado:renderResultado,custos:renderCustos,detalhe:renderDetalhe,conferencia:renderConferencia,config:renderConfig,ajuda:renderAjuda};
  $$('pageContent').innerHTML=(pages[state.view]||renderInicio)(reg,q);
  wireContent();$$('footMeta').hidden=state.view==='glossario';$$('footMeta').textContent=state.view==='dre'&&dreUI.base==='competencia'?`Competência informada por mês · leitura da nuvem: ${dataBR(state.updated)}. Preenchimento não equivale a fechamento contábil.`:`${state.records.length} registros mensais · ${state.D.accounts.length} contas · ${state.periodo} · ${q.rotulo} · Leitura da nuvem: ${dataBR(state.updated)}. Valores não conciliados não representam fechamento.`;
}
function renderInicio(reg,q){
 ...
 h+=resumoMovimento(reg)+comparativoMovimento(reg);
