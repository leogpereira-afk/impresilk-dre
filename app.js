...
let state={D:null,records:[],cfg:{},permissoes:{leitura:true,edicao:false,admin:false},view:'dre',periodo:'',comparar:'',consulta:'',grupo:'todos',tipo:'todos',ano:false,detCache:{},updated:null};
...
function renderInicio(reg,q){
 const latest=[...state.records].sort((a,b)=>monthSortKey(b.label)-monthSortKey(a.label))[0];
 let h=!reg?`<div class="month-empty"><div><b>${esc(state.periodo)} ainda não tem dados.</b><span>O histórico abaixo continua disponível para consulta.</span></div>${latest?`<button data-period="${esc(latest.label)}">Abrir ${esc(latest.label)} →</button>`:''}</div>`:'';
 h+=cfoResumo(reg)+resumoMovimento(reg)+comparativoMovimento(reg)+graficoRitmoAno();
 h+=`<div class="dashboard-grid">${graficoEvolucao()}${painelGrafico('Para onde foi o dinheiro',state.periodo+' · principais grupos de saídas',rankingCustos(reg),'<button data-go="custos" class="text-button">Ver todas →</button>')}</div>`;
 h+=`<div class="section-heading"><div><p class="eyebrow">DESPESAS DO DIA A DIA</p><h2>Energia, água e despesas fixas</h2></div><button data-go="custos">Abrir centros de custos →</button></div>`+atalhosCustos();
 h+=card('Conferência e próximos passos',`<div class="review-grid"><div><span class="label">Cobertura da base</span><b>${esc(q.rotulo)}</b></div><div><span class="label">Pendências de classificação</span><b>${(reg?.pendencias||[]).length}</b></div><div><span class="label">Saldo bancário disponível</span><b>Não apurado</b></div></div><div class="actions"><button data-go="conferencia">Conferir dados →</button><button data-go="caixa">Planejar o caixa →</button></div>`,false);
 return h;
}
function renderCaixa(reg){const r=F.resumo(reg);let h=graficoEvolucao();
  if(reg)h+=`<div class="cards">${metric('Recebimentos operacionais',r.operacionais,'Pagamentos recebidos de clientes','operacionais')}${metric('Saldo disponível',null,'Exige saldo inicial e conciliação bancária')}${metric('Variação do período',r.variacao,'Movimento da base salva','variacao')}</div>`+`<div class="split">${card('Entradas consideradas',linha('Recebimentos operacionais',r.operacionais,false,'operacionais')+linha('Empréstimos identificados',r.emprestimos,false,'emprestimos')+linha('Rendimentos',r.rendimentos,false,'rendimentos')+linha('Entradas a identificar',r.naoIdentificadas,false,'naoIdentificadas')+linha('Outras entradas / diferenças de detalhe',r.outrasEntradas,false,'outrasEntradas')+linha('Total',r.entradas,true,'entradas'))}${card('Saídas consideradas',linha('Pagamentos da operação',r.pagamentosOperacionais,false,'pagamentosOperacionais')+linha('Sócios e arrendamento',r.socios,false,'socios')+linha('Parcelas de ativos',r.parcelasAtivos,false,'parcelasAtivos')+linha('Dívidas classificadas',r.dividas,false,'dividas')+linha('Transferências entre empresas',r.transferencias,false,'transferencias')+linha('Investimentos classificados',r.investimentos,false,'investimentos')+linha('Cartão / saídas sem detalhe',r.pendentes,false,'pendentes')+linha('Total',r.saidas,true,'saidas'))}</div><p class="hint">A classificação histórica foi preservada. Parcelas exigem revisão de principal, juros e contrato; não são tratadas automaticamente como aquisição de patrimônio.</p>`;
...
function historicoTabela(){const year=state.periodo.split('/')[1];const recs=state.records.filter(r=>r.label.endsWith('/'+year));if(!recs.length)return '<p>Sem histórico neste ano.</p>';
 /* ACUMULADO DE JANEIRO ATÉ O MÊS ESCOLHIDO — a mesma régua do DRE mensal. Somava
    o ano inteiro (inclusive meses DEPOIS do escolhido) e passava por cima de mês
    sem dado: com fevereiro faltando, Jan + Mar virava "acumulado". Mês sem dado
    nunca vira zero; aqui ele suspende o total e diz qual falta. */
 const ateMes=PT_MON.indexOf(state.periodo.split('/')[0]);
 const esperados=PT_MON.slice(0,ateMes+1).map(m=>m+'/'+year);
 const doPeriodo=esperados.map(l=>recs.find(r=>r.label===l)||null);
 const faltam=esperados.filter((l,i)=>!doPeriodo[i]);
 const escopos=new Set(doPeriodo.filter(Boolean).map(r=>[r.company,r.basis,r.qualidade?.escopo,r.qualidade?.regra].join('|')));
 const bloqueio=faltam.length?`Sem acumulado: falta ${faltam.join(', ')}`:escopos.size>1?'Sem acumulado: empresas ou critérios diferentes no período':'';
 const total=bloqueio?null:(()=>{let e=0,s=0;for(const x of doPeriodo){const r=F.resumo(x);e+=Math.round(r.entradas*100);s+=Math.round(r.saidas*100);}return {e:e/100,s:s/100};})();
 return `<div class="table-scroll"><table><thead><tr><th>Mês</th><th>Cobertura</th><th class="num">Entradas</th><th class="num">Saídas</th><th class="num">Variação</th></tr></thead><tbody>${recs.map(x=>{const r=F.resumo(x);return `<tr><td>${esc(x.label)}</td><td>${esc(F.qualidade(x).rotulo)}</td><td class="num">${money(r.entradas)}</td><td class="num">${money(r.saidas)}</td><td class="num">${money(r.variacao)}</td></tr>`;}).join('')}<tr class="total"><td>Acumulado ${esc(esperados[0].split('/')[0])}–${esc(state.periodo)}</td><td>${bloqueio?esc(bloqueio):'Inclui períodos a conferir'}</td><td class="num">${total?money(total.e):'—'}</td><td class="num">${total?money(total.s):'—'}</td><td class="num">${total?money(Math.round((total.e-total.s)*100)/100):'—'}</td></tr></tbody></table></div><p class="hint">A soma do histórico não é saldo bancário. Meses parciais são identificados e não recebem conclusões automáticas de desempenho.</p>`;
}
function renderResultado(reg){const r=F.resumo(reg);return resumoMovimento(reg)+graficoPonte(reg)+card('Resultado por competência',`<div class="actions"><button data-go="dre">Abrir DRE mensal e competência →</button></div><div class="note"><h3>Consulte a apuração no DRE mensal</h3><p>Esta base contém recebimentos e pagamentos. A apuração por competência está no DRE mensal, com preenchimento revisado de receitas, custos, despesas, depreciação e tributos.</p></div><p>A leitura gerencial de caixa continua disponível abaixo, com critérios e origem explícitos.</p>`,false)+card('Demonstrativo gerencial de caixa',reg?`<p>${esc(reg.label)} · ${esc(F.qualidade(reg).rotulo)} · ${esc(reg.origem==='erp'?'Impresilk + Universo':reg.company||'Empresa não informada')}</p>${linha('Recebimentos operacionais',r.operacionais,false,'operacionais')}${linha('Pagamentos classificados na operação',-r.pagamentosOperacionais,false,'pagamentosOperacionais')}${linha('Saldo antes dos demais grupos',r.saldoOperacional,true,'saldoOperacional')}${linha('Sócios e arrendamento',-r.socios,false,'socios')}${linha('Parcelas de ativos',-r.parcelasAtivos,false,'parcelasAtivos')}${linha('Investimentos classificados',-r.investimentos,false,'investimentos')}${linha('Dívidas classificadas',-r.dividas,false,'dividas')}${linha('Transferências entre empresas',-r.transferencias,false,'transferencias')}${linha('Entradas de empréstimos',r.emprestimos,false,'emprestimos')}${linha('Rendimentos financeiros',r.rendimentos,false,'rendimentos')}${linha('Entradas a identificar e outras entradas',r.naoIdentificadas+r.outrasEntradas)}${linha('Saídas sem detalhamento',-r.pendentes,false,'pendentes')}${linha('Variação de caixa',r.variacao,true,'variacao')}<p class="hint">A decomposição fecha internamente. Não comprova conciliação com extrato nem lucro contábil.</p><div class="actions"><button id="printReport">Imprimir / salvar PDF</button><button data-go="detalhe">Ver contas</button></div>`:'<p>Selecione um período com dados.</p>');}
function renderDetalhe(reg){if(!reg)return '<div class="empty">Este mês não tem dados. Escolha um período do histórico.</div>';const cells=reg.cells||[],groups=cells.filter(c=>c.code.split('.').length===2);const query=state.consulta.toLocaleLowerCase('pt-BR');const filtered=cells.filter(c=>(state.tipo==='todos'||c.code.startsWith(state.tipo))&&(state.grupo==='todos'||c.code===state.grupo||c.code.startsWith(state.grupo+'.'))&&(!query||(c.name+' '+c.code).toLocaleLowerCase('pt-BR').includes(query)));const cmp=state.records.find(r=>r.label===state.comparar),comp=F.comparacao(reg,cmp);const sameMap=(reg.origem||'planilha')===(cmp?.origem||'planilha');
 return card('Contas, produtos e centros',`<div class="filterbar"><input id="searchAccounts" aria-label="Buscar conta ou produto" placeholder="Buscar conta, produto, energia, frota…" value="${esc(state.consulta)}"><select id="accountType" aria-label="Tipo de movimento"><option value="todos">Entradas e saídas</option><option value="1">Entradas</option><option value="2">Saídas</option></select><select id="accountGroup" aria-label="Categoria"><option value="todos">Todas as categorias</option>${groups.map(c=>`<option value="${esc(c.code)}">${esc(c.name)}</option>`).join('')}</select></div><p class="hint">${filtered.length} contas. Totais e subcontas são níveis da mesma árvore; não devem ser somados entre si.</p><div class="table-scroll"><table><thead><tr><th>Conta / produto</th><th class="num">${esc(reg.label)}</th><th class="num">${esc(state.comparar||'Comparação')}</th><th class="num">Variação</th></tr></thead><tbody>${filtered.map(c=>{const old=cmp?.cells?.find(x=>x.code===c.code),ok=comp.permitida&&!!old&&(sameMap||c.code.split('.').length<=2)&&old.name===c.name;return `<tr><td style="padding-left:${12+Math.min(c.code.split('.').length-1,5)*12}px"><button data-account="${esc(c.code)}">${esc(c.name)}</button><small> ${esc(c.code)}</small></td><td class="num">${botaoConta(c.code,money(c.value))}</td><td class="num">${ok?money(old.value):'—'}</td><td class="num">${ok?money(c.value-old.value):'—'}</td></tr>`;}).join('')}</tbody></table></div><p class="hint">${!state.comparar?'Escolha “Comparar com” no topo para ver a variação conta a conta.':comp.permitida?`Diferenças calculadas contra ${esc(state.comparar)}.${comp.ressalva?' A cobertura da coleta ainda não foi validada nos dois meses: o cálculo está feito, falta conferir se a base está completa antes de decidir.':''}${sameMap?' Conta renomeada ou ausente no outro mês fica em <b>—</b>.':` Os dois meses vieram de bases diferentes: abaixo do segundo nível as contas não se comparam e ficam em <b>—</b>.`}`:esc(comp.motivo||'')} Nomes são os de cada mês, preservados no histórico.</p><div class="actions"><button id="exportAccounts">Baixar contas em CSV</button></div>`)+card('Totais sem detalhamento',F.residuos(reg).length?F.residuos(reg).map(x=>linha(x.name+' · '+x.code,x.value)).join('')+'<p class="hint">Pode haver lançamento direto na conta-pai. Conferir antes de distribuir qualquer valor.</p>':'<p>Os totais com filhos fecham com o detalhamento disponível.</p>',false);
}
function renderConferencia(reg,q){const p=reg?.pendencias||[],dup=dedupePeriods(state.records).dropped;let h=faixaCobertura()+card('Saúde da informação',`<div class="fatos">${fato('Cobertura do período',q.rotulo)}${fato('Última coleta ou registro',dataBR(q.coletadoEm))}${fato('Conciliação bancária',reg?.qualidade?.conciliado?'Registrada':'Não comprovada')}${fato('Edições locais pendentes',String(getQueue().length))}</div>`+`<p class="hint">O botão “Atualizar Mubisys” solicita a coleta do mês atual e do anterior. Acompanhe a execução no aviso acima. “Ler base salva”, em Sistemas e configurações, apenas relê a nuvem.</p>`);
...
function renderAjuda(){return [
 ...
 ['Onde está cada recurso','Produtos, serviços, pessoal, materiais, máquinas, frota, energia e água estão em Detalhamento. Os gráficos de entradas e saídas estão em Visão geral e Caixa. Despesas por mês, como energia e água, estão em Centros de custos. Os demonstrativos de caixa e competência estão em DRE mensal, com planilha e impressão. As definições e fórmulas estão no Glossário financeiro. Backup e importação ficam em Sistemas e configurações.'],
...
function wireContent(){
  ...
  on('legacyBackup',()=>download('dre-fila-anterior.json',localStorage.getItem('impresilk_dre_queue')));on('printReport',()=>exportarCFO({tipo:'executivo',periodo:state.periodo},$$('printReport')));on('backupBtn',exportarBackup);on('addMonthBtn',()=>$$('monthFileInput').click());on('restoreBtn',()=>$$('restoreInput').click());on('auditMubi',conferirMubi);on('cloudReadBtn',()=>pullCloud(true));on('originEvents',()=>mostrarEventos());
  on('exportAccounts',()=>{const r=regAtual();download('dre-contas-'+safeId(r.label)+'.csv','\ufeffConta;Nome;Valor\n'+r.cells.map(c=>[c.code,c.name,Number(c.value).toFixed(2).replace('.',',')].map(celulaCSV).join(';')).join('\n'),'text/csv');});
...
function abrirConta(code){cfoVoltas=[];abrirAnaliseConta(code,state.periodo);}
...
function iniciarAcompanhamentoColeta(){
  ...
  clearInterval(coletaTimer);coletaControle.reset();coletaControle.consultar();
  coletaTimer=setInterval(()=>{if(!document.hidden&&AUTH.temCracha())coletaControle.consultar();},30000);
}
function pararAcompanhamentoColeta(){clearInterval(coletaTimer);coletaTimer=null;coletaControle?.reset();}
function configSegura(cfg){const allowed=['produtosCodigo','contasRemanejadas','previaERP','bancos','regras','demonstrativosCompetencia'];return Object.fromEntries(allowed.filter(k=>cfg[k]!=null).map(k=>[k,cfg[k]]));}
